...
legend({num2str(Freqs(1)),num2str(Freqs(2)),num2str(Freqs(3)),...
        num2str(Freqs(4)),num2str(Freqs(5)),num2str(Freqs(6)),...
       },'fontsize',14,'Location','best')
figure(2)
[sacn_angle,ff]=meshgrid(Freqs,scan_angle1);
p_all(p_all<-90) = -90;
surf(ff,sacn_angle,p_all);
set(gca,'view',[135 45])
zlim([-90 0]) 
xlim([-90 90])
xlabel('角度/^o','fontsize',14)
...
for nf =1 :length(Freqs)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    figure(3)
    plot(scan_angle1, p_opt,'-.')%,format(1,:)
    hold on
%     plot(angle_ml , p_ml,format(2,:))
%     plot(angle_sl , p_sl,format(3,:))
    ylim([-90 0])
