...
% FIR滤波器参数
L = 15;             % 滤波器长度L
tau = 0.12345*Ts;   % 期望延迟量τ
fc = 0.4*fs;        % 通带截止频率fc
f2 = 0.5*fs;        % 数字频带边缘值，无法到达
K = 100;            %离散化频率点数K

STEP_d = f2/fs/K;   % [0,0.5)分成100点，精度0.005
F_PB = linspace(0,fc/fs,fc/f2*K+1); % 延迟滤波器的通带F_PB = [0,0.4]，共81点
fd_ideal = F_PB;                    % 理想数字频带fd_ideal = [0,0.4]
fd = 0:STEP_d:(f2/fs-STEP_d);       % 数字频带fd = [0,0.5)，fd = f/fs 全频带

% 反向延迟D个节拍
if (tau>=-0.5*Ts && tau<0.5*Ts && mod(L,2)==1)  % L为奇数，τ∈[-0.5Ts,0.5Ts)
    D = (L-1)/2;
elseif (tau>=0 && tau<0.5*Ts && mod(L,2)==0)    % L为偶数，τ∈[0Ts,0.5Ts)
    D = L/2-1;
elseif (tau>=-0.5*Ts && tau<0 && mod(L,2)==0)   % L为偶数，τ∈[-0.5Ts,0)
	D = L/2+1;
end
   
%%%%%%%%%%%%%%%%%%%%% part2: FIR滤波器的期望频率响应 %%%%%%%%%%%%%%%%%%%%%%%
Hd_PB = exp(-1i*2*pi*fd_ideal*(D+tau/Ts)).'; 
Hd = [Hd_PB;zeros(K-length(F_PB),1)];  % (0.4,0.5) Hd设定为0，理想截止特性
Amplitude_Hd = 20*log10(abs(Hd_PB));
Phase_Hd = angle(Hd_PB)/pi*180;
...
%       
e_f = exp(-1i*2*pi*kron((0:L-1).',fd));

% L∞-norm:optimal Chebyshev filter formulation
cvx_begin
    variable h_Inf(L,1)
    minimize(norm(lambda_k.*(e_f.'*h_Inf - Hd),Inf))
cvx_end
...
cvx_begin
    variable h_1(L,1)
    minimize(norm(lambda_k.*(e_f.'*h_1 - Hd),1))
cvx_end
...
% 使用L1范数准则设计出的FIR滤波器
H_1 = e_f.'*h_1;
Amplitude_H_1 = 20*log10(abs(H_1));
...
cvx_begin
    variable h_2(L,1)
    minimize(norm(lambda_k.*(e_f.'*h_2 - Hd),2))
cvx_end
...
% 使用L2范数准则设计出的FIR滤波器
H_2 = e_f.'*h_2;
Amplitude_H_2 = 20*log10(abs(H_2));
...
for i=1:length(t)
	...
	s_delay_LPM(i) = sin(2*pi*(f1+(fu-f1)/(2*T)*(t(i)-tau))*(t(i)-tau));
end
...
xlabel('数字频率{\itf}_d');
ylabel('相位/(°)');
axis([0 f2/fs -200 200]);

% figure3: 设计频率响应误差
figure;
plot(fd_ideal,abs(H_Inf(1:length(F_PB))-Hd_PB),':k','LineWidth',2);    % L∞
hold on;
plot(fd_ideal,abs(H_1(1:length(F_PB))-Hd_PB),'--b','LineWidth',2);     % L1
hold on;
plot(fd_ideal,abs(H_2(1:length(F_PB))-Hd_PB),'-g','LineWidth',2);      % L2
legend('{\itL}_∞准则','{\itL}_1准则','{\itL}_2准则','Location','NorthWest');
xlabel('{\itf}_d');
ylabel('|{\itH}({\itf}_d)-{\itH}_d({\itf}_d)|');
axis([0 fc/fs 0 8*10^(-3)]);
